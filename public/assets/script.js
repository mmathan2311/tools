...
(function(){  
  ...
  if($('gst-calc')){
    ...
    $('gst-reverse').addEventListener('click',()=>{ const total = parseFloat($('gst-amount').value||0); if(!total){ alert('Enter the total (GST-inclusive) price in the Amount field above'); return; } const rate = parseFloat($('gst-rate').value||0); const type = $('gst-type').value; const base = total / (1 + rate/100); const tax = total - base; let cgst=0,sgst=0,igst=0; if(type==='intra'){ cgst=tax/2; sgst=tax/2; } else { igst = tax; } $('gst-result').style.display='block'; $('gst-result').innerHTML = `<div class="small" style="margin-bottom:6px">Treating ₹${formatCurrency(total)} as the GST-inclusive total:</div><div><strong>Base Amount:</strong> ₹${formatCurrency(base)}</div><div><strong>GST (${rate}%):</strong> ₹${formatCurrency(tax)}</div>${type==='intra'?`<div><strong>CGST:</strong> ₹${formatCurrency(cgst)} &nbsp; <strong>SGST:</strong> ₹${formatCurrency(sgst)}</div>`:`<div><strong>IGST:</strong> ₹${formatCurrency(igst)}</div>`}<div style="margin-top:8px"><strong>Total Price:</strong> ₹${formatCurrency(total)}</div>`; });
  }
